export default defineEventHandler(async (event) => {
  ...
  const rows = await queryD1<{ slug: string; title: string; description: string; pub_date: string; tags: string; draft: number }>(
    event, `SELECT slug,title,description,pub_date,tags,draft FROM posts WHERE draft=0 AND (title LIKE ? OR description LIKE ?) ORDER BY pub_date DESC LIMIT 20`, [like, like]
  )
