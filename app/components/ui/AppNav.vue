<template>
  ...
  <nav aria-label="主导航" class="fixed inset-x-0 top-0 z-50 border-b border-[var(--color-void-border)]" style="background:rgba(5,5,12,0.96);backdrop-filter:blur(10px)">
    ...
    <div :class="navMaxWidth" class="mx-auto h-full flex items-center justify-between gap-3 min-w-0">
...
const navMaxWidth = computed(() => {
  if (route.path.startsWith('/lab')) return 'max-w-5xl'
  if (route.path === '/' || route.path.startsWith('/explore') || ['/tags', '/search', '/stats'].some(path => route.path.startsWith(path)))
    return 'max-w-3xl'
  return 'max-w-2xl'
})
