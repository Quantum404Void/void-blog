...
description: "系统梳理静态与动态负载均衡算法，涵盖轮询、随机、权重、IP Hash、一致性 Hash、最少连接、最快响应等，并对比 Nginx、Dubbo、Ribbon 的实现差异。"
pubDate: 2023-11-15
...
| 算法 | Nginx | Dubbo | Spring Cloud Ribbon |
|------|-------|-------|---------------------|
| 轮询 | ✅ 默认 | ✅ | ✅ `RoundRobinRule` |
| 加权轮询 | ✅ `weight` | ✅ | ✅ `WeightedResponseTimeRule` |
| 随机 | ✅ | ✅ | ✅ `RandomRule` |
| IP Hash | ✅ `ip_hash` | ❌ | ❌ |
| URL Hash | ✅ `hash $uri` | ❌ | ❌ |
| 一致性 Hash | ✅ `consistent` | ✅ | ❌ |
| 最少连接 | ✅ `least_conn` | ❌ | ✅ `BestAvailableRule` |
| 最快响应 | ❌（商业版） | ✅ | ✅ `WeightedResponseTimeRule` |
| 故障重试 | ✅ `proxy_next_upstream` | ✅ | ✅ `RetryRule` |
