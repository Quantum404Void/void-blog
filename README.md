...
- 当架构、流程、数据关系或抽象概念仅靠文字不易理解时，优先加入示意图、流程图、数据图表或必要截图。
- 图片必须服务于解释，不使用与正文无关的装饰图；一张信息明确的图优于多张泛化配图。
- 每张图片应提供准确的替代文本；需要补充背景或结论时使用图注，并标明外部图片与数据来源。
- 图片需控制体积并提供合适尺寸，避免影响文章首屏速度和移动端阅读。
